...
func ParseFile(path string) (*BuildSpec, error) {
	...
	for scanner.Scan() {
		...
		if len(line) == 1 {
			...
			return nil, fmt.Errorf("buildspec contains more than a server name and a MAC address: got %d lines", len(line))
		}
	}
	...
	if len(spec.Hosts) != len(spec.MACs) && len(spec.MACs) != 0 {
		return nil, fmt.Errorf("buildspec has an uneven number of hosts and MACs")
	}
